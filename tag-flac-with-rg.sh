...
# If there is ReplayGain in any of them, skip
if [ "`metaflac --list "$1"/*.flac | grep REPLAYGAIN`" != "" ]
then
	echo $1" (ReplayGain already applied, moving on)"
	exit 0
fi
